...
//Parameters in the Exponential potential
a=0.93*10^-15
A=111*10^6*e
...
function tol=f(E)
    ...
    ub=ode([1D-5;-8D-3],pmax,pb,f_ode)
    ufm=uf(1,n1)
    dufm=uf(2,n1)
    ubm=ub(1,n2)
    dubm=ub(2,n2)
...
plot(pf,uf(1,:),'r','LineWidth',2)
plot(pb,ub(1,:),'b','LineWidth',2)
xtitle('Shooting-Matching u for Exponential potential','Dimensionless variable p','Wavefunction u')
...
xtitle('Shooting matching R for Exponential potential','Dimensionless variable p','Radial wavefunction R')
legend('Forward solution','Backward solution')
aa=gca
