...
for i=1:N
    ...
    V_LJ(i,i)=C0+(C1/(x(i)^12))-(C2/(x(i)^6))
    V1_LJ(i)=V_LJ(i,i) 
end
...
//bw perturbation
Pn=psi_RHO(:,1)*(psi_RHO(:,1))'//Projection operator
Qn=eye(N,N)-Pn
En=E1_LJ(1)
...
legend("$u_{\text{RHO}}$","$u_{\text{LJ (12-6)}}$","$u_{\text{BW perturbation}}$")
lll = gce(); 
lll.legend_location = "in_upper_right"; 
lll.font_size = 4;      
aaa = gca()
aaa.font_size = 4; 
csvWrite([r psi_RHO_norm(:,1) psi_LJ_norm(:,1) psi_n_norm ],"LJ2data.csv")
