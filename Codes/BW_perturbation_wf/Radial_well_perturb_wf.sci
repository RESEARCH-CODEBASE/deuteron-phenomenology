...
//Parameters in the Radial well potential
L=2.32*10^-15 //Width of the well
V0_RW=-29.2*10^6//Depth of the potential well in eV
...
H_RW=(((-hbar^2/(2*m*e*h^2))*T)+V_RW)/(10^6)
[vec_RW,E_RW]=spec(H_RW)
...
for i=1:N
    if E_RW(i,i)<0
        count=count+1
        E1_RW(count)=E_RW(i,i)
        phi_RW(:,count)=vec_RW(:,i)
    end
end
...
scf(0)
clf(0)
plot(x/10^-15,V1_RW/10^6,'LineWidth',2)
xtitle('Radial Well','r (fm)','Energy in MeV')
aaa=gca
aaa.data_bounds=[0,-35;5,10]
aaa.x_label.font_size = 4
aaa.y_label.font_size = 4
aaa.title.font_size = 4

//results
disp("no. of bound states, for this Radial well potential = "+string(count))
disp("Radial well Bound state energy:")
disp(E1_RW)
disp("Size of the nucleus")
[val,index]=max(phi_RW(:,1).*phi_RW(:,1))
disp(x(index))
...
plot(x,phi_RW(:,1),'r','LineWidth',2)
legend(['RHO';'Radial well']);
xtitle("RHO vs Radial well wavefunctions (u)",'r','u')
aaa=gca
aaa.x_label.font_size = 4
aaa.y_label.font_size = 4
aaa.title.font_size = 4
...
figure(2)
clf()
plot(x,V1_RW,'--','LineWidth',2)
plot(r,V1_RHO,'r','LineWidth',2)
legend(['Radial well';'RHO']);
xtitle("RHO vs Radial well Potentials",'r','V')
aaa=gca
aaa.x_label.font_size = 4
aaa.y_label.font_size = 4
aaa.title.font_size = 4
...
psi_RW_norm=phi_RW(:,1)./sqrt(inttrap(x,phi_RW(:,1).*phi_RW(:,1)))
psi_n_norm=psi_n./sqrt(inttrap(r,psi_n.*psi_n))
plot(r/(1D-15),psi_RHO_norm(:,1)/(3.162D+7),'--','LineWidth',2)
plot(r/(1D-15),psi_RW_norm(:,1)/(3.162D+7),'r--','LineWidth',2)
...
legend("$u_{\text{RHO}}$","$u_{\text{Radial Well}}$","$u_{\text{BW perturbation}}$")
lll = gce(); 
lll.legend_location = "in_upper_right"; 
lll.font_size = 4;     
aaa = gca()
aaa.font_size = 4; 
csvWrite([r psi_RHO_norm(:,1) psi_RW_norm(:,1) psi_n_norm ],"RWdata.csv")
