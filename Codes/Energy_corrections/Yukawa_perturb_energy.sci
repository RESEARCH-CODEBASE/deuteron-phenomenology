...
mp=1.6726*10^-27 //mass of proton
mn=1.6749*10^-27 //mass of neut
...
r_max=30*10^-15//xmax
r=linspace(1D-17,r_max,1001)
V_RHO=V0_RHO+(m*omega^2*(r.^2)/(2))
f=(V0-(C0*exp(-r/r0)./r))-V_RHO
n=25
...
for(k=1:n-1)
    ...
    correction=((H2k0)^2)/(epsilon(1)-epsilon(k+1))
    sumk=sumk+correction
end
...
disp("retrieved energy for Yukawa potential")
